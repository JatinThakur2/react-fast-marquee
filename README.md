...
| `darkModeGradient` | `{ enabled: boolean; color: string; gradientWidth?: number }` | `null` | Allows the marquee to display a gradient in dark mode. If enabled, it will apply a gradient background based on the provided color and width.                                        |
